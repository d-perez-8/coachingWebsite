...
var RelativeCaption = /** @class */ (function () {
  function RelativeCaption(instance) {
    ...
    this.core.settings = __assign(
      __assign({}, this.core.settings),
      defaultSettings,
    );
    // extend module default settings with lightGallery core settings
    this.settings = __assign(
      __assign(
        __assign({}, lg_relative_caption_settings_1.relativeCaptionSettings),
        this.core.settings,
      ),
      defaultSettings,
    );
  ...
  RelativeCaption.prototype.init = function () {
    ...
    this.core.LGel.on(
      lg_events_1.lGEvents.slideItemLoad + ".caption",
      function (event) {
        var _a = event.detail,
          index = _a.index,
          delay = _a.delay;
    ...
    this.core.LGel.on(
      lg_events_1.lGEvents.afterSlide + ".caption",
      function (event) {
    ...
    this.core.LGel.on(
      lg_events_1.lGEvents.beforeSlide + ".caption",
      function (event) {
        var index = event.detail.index;
        setTimeout(function () {
          var slide = _this.core.getSlideItem(index);
    ...
    this.core.LGel.on(
      lg_events_1.lGEvents.containerResize + ".caption",
      function (event) {
        _this.setRelativeCaption(_this.core.index);
      },
    );
  };
  RelativeCaption.prototype.setCaptionStyle = function (
    index,
    rect,
    slideWrapRect,
  ) {
