...
function isValidFormatter(entry: unknown): entry is Formatter {
  return (
    ...
    typeof (<Formatter>entry).from === "function"
  );
}
...
function isValidPartialFormatter(entry: unknown): entry is PartialFormatter {
  ...
  return (
    typeof entry === "object" && typeof (<Formatter>entry).to === "function"
  );
}
...
class Spectrum {
  ...
  private handleStepPoint(i: number, n: number | false): void {
    ...
    const totalSteps =
      (this.xVal[i + 1] - this.xVal[i]) / (this.xNumSteps[i] as number);
...
function testKeyboardPageMultiplier(
...
function testTooltips(
  parsed: ParsedOptions,
  entry: boolean | Formatter | (boolean | Formatter)[],
): void {
  if (entry === false) {
    return;
  }
...
function testHandleAttributes(
  parsed: ParsedOptions,
  entry: HandleAttributes[],
): void {
  if (entry.length !== parsed.handles) {
...
function testOptions(options: Options): ParsedOptions {
  ...
  const tests: {
    [key in keyof Options]: {
      r: boolean;
      t: (parsed: ParsedOptions, entry: unknown) => void;
    };
  } = {
  ...
  const msPrefix =
    (d.style as CSSStyleDeclarationIE10).msTransform !== undefined;
  ...
  parsed.style = styles[parsed.dir][parsed.ort] as
    | "left"
    | "top"
    | "right"
    | "bottom";
...
function scope(
  ...
  function addTooltip(
    ...
    return addNodeTo(
      handle.firstChild as HTMLElement,
    ...
    bindEvent(
      ...
      function (values, handleNumber, unencoded) {
        ...
        if (options.tooltips[handleNumber] !== true) {
          formattedValue = (<Formatter>options.tooltips[handleNumber]).to(
            unencoded[handleNumber],
          );
        }

        (<HTMLElement>scope_Tooltips[handleNumber]).innerHTML = <string>(
          formattedValue
        );
      },
    );
    ...
    bindEvent(
      ...
      function (values, handleNumber, unencoded, tap, positions) {
        ...
        scope_HandleNumbers.forEach(function (index) {
          ...
          let min: number | string = <number>(
            checkHandlePosition(scope_Locations, index, 0, true, true, true)
          );
          let max: number | string = <number>(
            checkHandlePosition(scope_Locations, index, 100, true, true, true)
          );
  ...
  function addMarking(
    ...
    function getClasses(type: PipsType, source: string) {
      ...
      const orientationClasses = a
        ? valueOrientationClasses
        : markerOrientationClasses;
      const sizeClasses = a ? valueSizeClasses : markerSizeClasses;

      return (
        source + " " + orientationClasses[options.ort] + " " + sizeClasses[type]
      );
    }
  ...
  function baseSize(): number {
    ...
    const alt = ("offset" + ["Width", "Height"][options.ort]) as
      | "offsetWidth"
      | "offsetHeight";
  ...
  function attachEvent(
    ...
    const method: EventHandler = function (
      event: BrowserEvent,
    ): false | undefined {
  ...
  function fixEvent(
    ...
    if (touch) {
      ...
      if (e.type === "touchstart") {
        const targetTouches: Touch[] = Array.prototype.filter.call(
          e.touches,
          isTouchOnTarget,
        );
        ...
        const targetTouch = (Array.prototype as any).find.call(
          e.changedTouches,
          isTouchOnTarget,
        );
  ...
  function getClosestHandle(clickedPosition: number): number | false {
    ...
    scope_Handles.forEach(function (handle, index) {
      ...
      const differenceWithThisHandle = Math.abs(
  ...
  function bindSliderEvents(behaviour: Behaviour) {
    ...
    if (!behaviour.fixed) {
      scope_Handles.forEach(function (handle, index) {
        ...
        attachEvent(
          actions.start,
          handle.children[0] as HTMLElement,
          eventStart,
          {
            handleNumbers: [index],
          },
  ...
  function updateHandlePosition(handleNumber: number, to: number): void {
    ...
    (scope_Handles[handleNumber].style as CSSStyleDeclarationIE10)[
      options.transformRule
    ] = translateRule;
  ...
  function updateConnect(index: number): void {
    ...
    ((<HTMLElement>scope_Connects[index]).style as CSSStyleDeclarationIE10)[
      options.transformRule
  ...
  function resolveToValue(
    to: null | false | undefined | string | number,
    handleNumber: number,
  ): number {
