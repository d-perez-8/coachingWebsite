...
const commands = ["get", "grant", "list", "revoke", "set"];
...
class Access extends BaseCommand {
  ...
  static name = "access";
  static params = ["json", "otp", "registry"];
    ...
    switch (cmd) {
      case "grant":
        if (!["read-only", "read-write"].includes(subcmd)) {
          throw this.usageError(
            "grant must be either `read-only` or `read-write`",
          );
        }
        if (!args[0]) {
  ...
  async #listCollaborators(pkg, user) {
    ...
    const collabs = await libnpmaccess.getCollaborators(
      pkgName,
  ...
  async #getStatus(pkg) {
    const pkgName = await this.#getPackage(pkg, false);
    const visibility = await libnpmaccess.getVisibility(
      pkgName,
      this.npm.flatOptions,
    );
    ...
    if (!name) {
      try {
        const pkg = await readPackageJson(
          path.resolve(this.npm.prefix, "package.json"),
        );
        name = pkg.name;
      } catch (err) {
        if (err.code === "ENOENT") {
          throw Object.assign(
            new Error("no package name given and no package.json found"),
            {
              code: "ENOENT",
            },
          );
