...
class VerifySignatures {
  ...
  async run() {
    ...
    const { edges, registries } = this.getEdgesOut(
      this.tree.inventory.values(),
      this.filterSet,
    );
    if (edges.size === 0) {
    ...
    await Promise.all(
      [...registries].map((registry) => this.setKeys({ registry })),
    );
    ...
    if (!this.auditedWithKeysCount) {
      throw new Error(
        "found no dependencies to audit that where installed from " +
    ...
    const timing =
      `audited ${this.auditedWithKeysCount} package${auditedPlural} in ` +
  ...
  getValidPackageInfo(edge) {
    ...
    if (
      node.isWorkspace || // Skip local workspaces packages
...
class Audit extends ArboristWorkspaceCmd {
  ...
  async auditSignatures() {
    ...
    if (opts.workspaces && opts.workspaces.length) {
      filterSet = arb.workspaceDependencySet(
        tree,
        opts.workspaces,
