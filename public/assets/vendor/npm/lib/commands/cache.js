...
const searchCachePackage = async (path, parsed, cacheKeys) => {
  ...
  const searchPack = new RegExp(
    `^make-fetch-happen:request-cache:.*/${parsed.escapedName}$`,
  );
    ...
    for (const ver of Object.keys(packument.versions)) {
      if (semver.satisfies(ver, parsed.rawSpec)) {
        if (
          ...
          cacheKeys.has(
            `make-fetch-happen:request-cache:${packument.versions[ver].dist.tarball}`,
          )
        ) {
          results.add(
            `make-fetch-happen:request-cache:${packument.versions[ver].dist.tarball}`,
...
class Cache extends BaseCommand {
  ...
  async verify() {
    ...
    const prefix =
      cache.indexOf(process.env.HOME) === 0
