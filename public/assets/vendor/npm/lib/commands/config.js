...
class Config extends BaseCommand {
  ...
  async set(args) {
    ...
    for (const [key, val] of Object.entries(keyValues(args))) {
      ...
      if (
        !this.npm.config.definitions[baseKey] &&
        !nerfDarts.includes(baseKey)
      ) {
  ...
  async get(keys) {
    ...
    for (const key of keys) {
      if (!publicVar(key)) {
        throw new Error(
          `The ${key} option is protected, and can not be retrieved in this way`,
        );
      }
...
`
  ...
  async fix() {
    ...
    return await Promise.all(
      locations.map((location) => this.npm.config.save(location)),
    );
  }
    ...
    if (!this.npm.global) {
      ...
      if (pkg.publishConfig) {
        ...
        for (const k of pkgKeys) {
          const v = publicVar(k)
            ? JSON.stringify(pkg.publishConfig[k])
            : "(protected)";
          msg.push(`${k} = ${v}`);
        }
        msg.push("");
