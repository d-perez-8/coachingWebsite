...
class Doctor extends BaseCommand {
  ...
  async exec(args) {
    ...
    const outHead = ["Check", "Value", "Recommendation/Notes"].map((h) =>
      this.npm.chalk.underline(h),
    );
    let allOk = true;
  ...
  async getLatestNpmVersion() {
    ...
    try {
      const latest = (await pacote.manifest("npm@latest", this.npm.flatOptions))
        .version;
    ...
    try {
      ...
      for (const f of files) {
        ...
        try {
          ...
          ok = false;
          const msg = `Missing permissions on ${f} (expect: ${maskLabel(
            mask,
          )})`;
  ...
  async verifyCachedFiles() {
    ...
    try {
      ...
      const { badContentCount, reclaimedCount, missingContent, reclaimedSize } =
        stats;
      if (badContentCount || reclaimedCount || missingContent) {
        if (badContentCount) {
          tracker.warn(
            "verifyCachedFiles",
            `Corrupted content removed: ${badContentCount}`,
          );
        }
