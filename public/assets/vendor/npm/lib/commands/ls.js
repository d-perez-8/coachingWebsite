  ...
  async exec(args) {
    ...
    const filterBySelectedWorkspaces = (edge) => {
      ...
      if (edge.from.isProjectRoot) {
        return (
          edge.to && edge.to.isWorkspace && wsNodes.includes(edge.to.target)
        );
      }
    ...
    const [rootError] = tree.errors.filter(
      (e) =>
...
const isExtraneous = (node, { global }) => node.extraneous && !global;
...
const getHumanOutputItem = (node, { args, color, global, long }) => {
  ...
  const label =
    ...
    (node[_dedupe] ? " " + (color ? chalk.gray("deduped") : "deduped") : "") +
    (invalid ? " " + (color ? chalk.red.bgBlack(invalid) : invalid) : "") +
...
const appendExtraneousChildren = ({ node, seenPaths }) =>
  ...
  [...node.children.values()].filter(
    (i) => !seenPaths.has(i.path) && i.extraneous,
  );
...
const mapEdgesToNodes =
  ...
  (edge) => {
    ...
    if (edge.invalid) {
      ...
      node[_invalid] =
        (node[_invalid] ? node[_invalid] + ", " : "") + `${spec} from ${from}`;
    }
...
const filterByPositionalArgs = (args, { node }) =>
  args.length > 0
    ? args.some((spec) => node.satisfies && node.satisfies(spec))
    : true;

const augmentNodesWithMetadata =
  ({ args, currentDepth, nodeResult, seenNodes }) =>
  (node) => {
    ...
    if (seenNodes.has(node.path)) {
      ...
      const targetLocation = root
        ? relative(root.realpath, realpath)
        : node.targetLocation;
      node = {
        name: node.name,
