...
class Outdated extends ArboristWorkspaceCmd {
  ...
  async exec(args) {
    ...
    await Promise.all(
      Array.from(this.edges).map((edge) => {
  ...
  getWorkspacesEdges(node) {
    ...
    for (const edge of this.tree.edgesOut.values()) {
      const workspace =
        edge && edge.to && edge.to.target && edge.to.target.isWorkspace;
  ...
  async getOutdatedInfo(edge) {
    ...
    try {
      ...
      if (
        !(err.code === "ETARGET" || err.code === "E403" || err.code === "E404")
      ) {
        throw err;
      }
    }
  ...
  makeJSON(list) {
    const out = {};
    list.forEach((dep) => {
      const { name, current, wanted, latest, path, type, dependent, homepage } =
        dep;
      out[name] = {
        current,
        wanted,
