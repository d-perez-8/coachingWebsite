...
class Profile extends BaseCommand {
  ...
  async set(args) {
    ...
    const readPasswords = async () => {
      ...
      const confirmedpassword =
        await readUserInfo.password("       Again:     ");
    ...
    if (writableProfileKeys.indexOf(prop) === -1) {
      throw new Error(
        `"${prop}" is not a property we can set. ` +
    ...
    const result = await otplease(this.npm, conf, (conf) =>
      npmProfile.set(newUser, conf),
    );
  ...
  async enable2fa(args) {
    ...
    const creds = this.npm.config.getCredentialsByURI(
    ...
    const interactiveOTP = await readUserInfo.otp(
