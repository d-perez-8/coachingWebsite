...
class Publish extends BaseCommand {
  ...
  async exec(args) {
    ...
    if (!dryRun) {
      await otplease(this.npm, opts, (opts) =>
        libpub(manifest, tarballData, opts),
      );
    }
    ...
    for (const [name, workspace] of this.workspaces.entries()) {
      ...
      try {
        ...
        if (err.code === "EPRIVATE") {
          log.warn(
            "publish",
            `Skipping workspace ${this.npm.chalk.green(
              name,
            )}, marked as ${this.npm.chalk.bold("private")}`,
          );
          continue;
        }
        throw err;
