...
class Shrinkwrap extends BaseCommand {
  ...
  async exec() {
    ...
    if (this.npm.global) {
      const er = new Error(
        "`npm shrinkwrap` does not work for global packages",
      );
    ...
    if (meta.hiddenLockfile) {
      meta.lockfileVersion =
        arb.options.lockfileVersion || meta.originalLockfileVersion;
    }
    ...
    const updatedVersion =
      meta.originalLockfileVersion !== meta.lockfileVersion
        ? meta.lockfileVersion
        : null;
    ...
    if (newFile) {
      ...
      log.notice(
        "",
        `npm-shrinkwrap.json updated to version ${updatedVersion}`,
      );
    } else {
