...
class Npm extends EventEmitter {
  ...
  async cmd(cmd) {
    ...
    if (this.config.isDefault("location") && !this.config.get("global")) {
      const hasPackageJson = await fs
        .stat(resolve(this.config.localPrefix, "package.json"))
        .then((st) => st.isFile())
        .catch(() => false);
      if (hasPackageJson) {
  ...
  async exec(cmd, args) {
    ...
    const implicitWorkspace =
      this.config.get("workspace", "default").length > 0;
    ...
    if (this.config.get("usage")) {
      ...
      if (this.global) {
        execPromise = Promise.reject(
          new Error("Workspaces not supported for global packages"),
        );
      } else {
  ...
  async [_load]() {
    ...
    await this.time("npm:load:mkdirplogs", () =>
      fs
        ...
        .catch((e) =>
          log.verbose("logfile", `could not create logs-dir: ${e}`),
        ),
    );
  ...
  get global() {
    return (
      this.config.get("global") || this.config.get("location") === "global"
    );
  }
