...
const messageText = (msg) =>
  msg.map((line) => line.slice(1).join(" ")).join("\n");
const indent = (val) =>
  Array.isArray(val) ? val.map((v) => indent(v)) : `    ${val}`;
  ...
  if (hasLoadedNpm) {
    ...
    if (logMethod) {
      ...
      if (logFiles.length) {
        ...
        message.push(
          `Log files were not written due to the config logs-max=${logsMax}`,
        );
      } else {
...
const exitHandler = (err) => {
  ...
  if (err) {
    ...
    const isShellout =
      npm.commandInstance && npm.commandInstance.constructor.isShellout;
    const quietShellout =
      isShellout && typeof err.code === "number" && err.code;
    if (quietShellout) {
      exitCode = err.code;
      noLogMessage = true;
      ...
      for (let [file, content] of files) {
        ...
        try {
          ...
          log.warn(
            "",
            `Could not write error message to ${file} due to ${err}`,
          );
        }
      }
  ...
  showLogFileError =
    (hasLoadedNpm && npm.silent) || noLogMessage ? false : !!exitCode;
