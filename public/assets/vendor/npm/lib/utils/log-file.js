...
const padZero = (n, length) =>
  n.toString().padStart(length.toString().length, "0");
...
class LogFiles {
  ...
  constructor({ maxLogsPerFile = 50_000, maxFilesPerProcess = 5 } = {}) {
  ...
  static format(count, level, title, ...args) {
    ...
    return format(...args)
      .split(/\r?\n/)
      .reduce(
        (lines, line) => (lines += prefix + (line ? " " : "") + line + os.EOL),
        "",
      );
  }
  ...
  #openLogFile() {
    ...
    try {
      ...
      const f = this.#getLogFilePath(
        padZero(count, this.#MAX_FILES_PER_PROCESS),
      );
